...
export const parentApi = {
    ...
    linkStudent: async (studentId: string, licenseKey?: string) => {
        const res = await fetch(`${API_URL}/students/link`, {
            method: 'POST',
            headers: getHeaders(),
            body: JSON.stringify({ studentId, licenseKey })
        });
        const data = await parseResponse(res);
        if (!res.ok) throw data;
        return data;
    ...
    activateLicense: async (studentId: string, licenseKey: string) => {
        const res = await fetch(`${API_URL}/parent/activate-license`, {
            method: 'POST',
            headers: {
                ...getHeaders(),
