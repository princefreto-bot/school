...
export const LinkStudentModal: React.FC<LinkStudentModalProps> = ({ isOpen, onClose, onSuccess }) => {
    ...
    const handleLink = async (studentId: string) => {
        ...
        setError('');
        try {
            await parentApi.linkStudent(studentId, licenseKey.trim());
    ...
    return (
        <div className="fixed inset-0 z-50 overflow-y-auto bg-slate-900/60 backdrop-blur-sm p-4 flex justify-center items-start sm:items-center min-h-screen">
            <div className="bg-white rounded-3xl shadow-2xl w-full max-w-lg my-auto overflow-hidden animate-in fade-in zoom-in duration-200">
                    ...
                    <div className="space-y-4">
                        ...
                        <div className="space-y-1.5">
                            <label className="block text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest">
                                Clé de licence / Clé d'activation (Optionnel)
                            </label>
                            <input
                                type="text"
                                value={licenseKey}
                                onChange={(e) => setLicenseKey(e.target.value)}
                                placeholder="Saisir la clé si l'enfant est déjà lié à un autre parent"
                                className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl text-slate-800 focus:outline-none focus:ring-2 focus:ring-blue-500 transition-all font-medium uppercase font-mono text-sm"
                            />
                            <p className="text-[10px] text-slate-400">
                                💡 Un enfant ne peut pas être lié 2 fois à moins de fournir la clé de licence qui l'a activé.
                            </p>
                        </div>
                    </div>
