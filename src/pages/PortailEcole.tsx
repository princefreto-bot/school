...
import { GraduationCap, Lock, Mail, Phone, Store, UserCheck, ShieldAlert, ArrowRight } from 'lucide-react';
...
export const PortailEcole: React.FC = () => {
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 flex items-center justify-center font-['Poppins'] p-4 relative overflow-hidden">
      {/* Effets lumineux premium */}
      <div className="absolute top-[-10%] right-[-10%] w-[50%] h-[50%] bg-amber-500/10 rounded-full blur-[100px] pointer-events-none" />
      <div className="absolute bottom-[-10%] left-[-10%] w-[50%] h-[50%] bg-blue-600/10 rounded-full blur-[100px] pointer-events-none" />

      <div className="w-full max-w-md bg-slate-900/40 backdrop-blur-xl border border-slate-800 rounded-3xl p-6 md:p-10 shadow-2xl relative z-10 animate-in fade-in zoom-in-95 duration-300">
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-slate-400 text-xs md:text-sm mt-1">Espace réservé à l'administration de l'école.</p>
        </div>
        ...
        <form onSubmit={handleLogin} className="space-y-4">
          ...
          {fetchError && (
            <div className="p-3 bg-rose-500/10 border border-rose-500/20 text-rose-400 rounded-2xl text-xs text-center font-bold">
          ...
          <div className="bg-slate-950/40 border border-slate-800/80 rounded-2xl p-4 space-y-3">
            ...
            {!isGlobalAccess && (
              <div className="relative animate-in slide-in-from-top-2 duration-200">
                ...
                <select 
                  className="w-full pl-11 pr-4 py-3 bg-slate-950/60 border border-slate-800 focus:border-amber-500 rounded-2xl text-xs md:text-sm font-bold text-slate-300 appearance-none focus:outline-none transition-colors"
                  value={selectedSchool} 
                  onChange={(e) => setSelectedSchool(e.target.value)} 
                  required={!isGlobalAccess}
                >
                  <option value="" disabled>-- Sélectionnez votre école --</option>
                  {schools.map(s => <option key={s.slug} value={s.slug} className="bg-slate-900">{s.name}</option>)}
                </select>
              </div>
            )}
          </div>
          ...
          <div className="space-y-3">
            <div className="relative">
              ...
              <input 
                ...
                className="w-full pl-11 pr-4 py-3 bg-slate-950/60 border border-slate-800 focus:border-amber-500 rounded-2xl text-xs md:text-sm focus:outline-none transition-colors"
                value={username} 
          ...
          {trialExpiredSchool && (
            <div className="p-3.5 bg-amber-500/10 border border-amber-500/30 rounded-2xl text-left">
              <p className="text-amber-500 font-extrabold text-xs">⏰ Période d'essai expirée</p>
              <p className="text-slate-300 text-[10px] mt-1">"{trialExpiredSchool}" doit régler son abonnement auprès du SuperAdmin pour réactiver l'accès.</p>
            </div>
          )}
        ...
        <div className="mt-8 pt-6 border-t border-slate-800/60 flex flex-col gap-4 text-center">
          <button 
