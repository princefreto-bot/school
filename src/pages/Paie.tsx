...
import {
  Wallet, Loader2, Send, AlertTriangle, CheckCircle, Printer,
  Sparkles, Plus, X, Users, Pencil, IdCard
} from 'lucide-react';
...
export const Paie: React.FC = () => {
  ...
  // ── Édition des infos de paie d'un salarié (fiche personnel) ──
  const emptyPaieForm: PaieInfoForm = {
    matricule: '', numeroCnss: '', dateEmbauche: '', modePaiement: '', compteBancaire: '', departement: '',
  };
  const [editStaff, setEditStaff] = useState<StaffMember | null>(null);
  const [paieForm, setPaieForm] = useState<PaieInfoForm>(emptyPaieForm);
  const [savingPaie, setSavingPaie] = useState(false);
  const [paieError, setPaieError] = useState('');
  ...
  const openEditStaff = (s: StaffMember) => {
    ...
    setPaieForm({
      ...
      numeroCnss: s.numeroCnss || '',
      dateEmbauche: s.dateEmbauche ? String(s.dateEmbauche).slice(0, 10) : '',
  ...
  const savePaieInfo = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setPaieError(data.error || 'Erreur lors de l\'enregistrement.');
      }
    } catch {
      setPaieError('Erreur réseau.');
    } finally {
      setSavingPaie(false);
    }
  };
  ...
  // ── Impression / export PDF via le modèle premium (react-to-print) ──
  ...
  const doPrint = useReactToPrint({
    contentRef: printRef,
    documentTitle: printSlip
      ? `Bulletin_${(printSlip.personnel?.nom || 'employe').replace(/\s+/g, '_')}_${printSlip.periode}`
      : 'Bulletin_de_paie',
    pageStyle: `
  ...
  // Déclenche l'impression une fois le bulletin monté dans le DOM
  useEffect(() => {
  ...
  return (
    <div className="space-y-6 pb-20 max-w-[1600px] mx-auto animate-slideUp">
      ...
      <div className="bg-white dark:bg-slate-900 rounded-3xl shadow-sm border border-slate-100 dark:border-slate-800 overflow-hidden">
        <div className="px-6 py-6 border-b border-slate-100 dark:border-slate-800 flex items-center gap-3">
          ...
          <span className="ml-auto text-[10px] text-slate-400 font-medium hidden sm:block">
            Renseignez matricule, CNSS, banque… pour chaque salarié — ces infos figurent sur le bulletin.
          </span>
        </div>
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-slate-50/50 dark:bg-slate-800/50 text-slate-400 dark:text-slate-500 text-[9px] font-black uppercase tracking-[0.1em] border-b border-slate-100 dark:border-slate-800">
                ...
                <th className="px-6 py-4 text-right">Infos paie</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-slate-50 dark:divide-slate-800/50">
              {loadingStaff ? (
                ...
                <tr><td colSpan={5} className="px-6 py-16 text-center text-slate-400 text-sm font-medium">Aucun membre du personnel.</td></tr>
              ) : (
                staff.map((s) => (
                  <tr key={s.id} className="hover:bg-slate-50/50 dark:hover:bg-slate-800/30 transition-colors">
                    ...
                    <td className="px-6 py-4 text-xs text-slate-500 dark:text-slate-400">
                      {s.matricule || <span className="text-slate-300 dark:text-slate-600">—</span>}
                    </td>
                    ...
                    <td className="px-6 py-4 text-right">
                      <button onClick={() => openEditStaff(s)} className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 text-slate-600 dark:text-slate-300 rounded-xl text-[10px] font-black uppercase tracking-widest transition ml-auto">
      ...
      {/* ── MODALE : ÉDITION DES INFOS DE PAIE DU SALARIÉ ── */}
      {editStaff && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-sm" onClick={() => setEditStaff(null)}>
          <div className="w-full max-w-lg bg-white dark:bg-slate-900 rounded-3xl shadow-2xl border border-slate-100 dark:border-slate-800 overflow-hidden" onClick={(e) => e.stopPropagation()}>
            <div className="px-6 py-5 border-b border-slate-100 dark:border-slate-800 flex items-center gap-3">
              <IdCard className="w-5 h-5 text-indigo-500" />
              <div>
                <h3 className="font-black text-slate-900 dark:text-white">Informations de paie</h3>
                <p className="text-[11px] text-slate-400 font-medium">{editStaff.nom} — {editStaff.role}</p>
              </div>
              <button onClick={() => setEditStaff(null)} className="ml-auto p-2 text-slate-400 hover:text-slate-600"><X className="w-4 h-4" /></button>
            </div>
            ...
            <div className="p-6 space-y-4">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <input value={paieForm.compteBancaire} onChange={(e) => setPaieForm({ ...paieForm, compteBancaire: e.target.value })} placeholder="Ex: TG053 0123 4567" className="w-full p-3 bg-slate-50 dark:bg-slate-950/40 border border-slate-200 dark:border-slate-800 rounded-xl text-slate-800 dark:text-slate-200 text-xs outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500" />
                </div>
              </div>
            </div>

            <div className="px-6 py-4 border-t border-slate-100 dark:border-slate-800 flex justify-end gap-3">
              <button onClick={() => setEditStaff(null)} className="px-4 py-2.5 text-xs font-black uppercase text-slate-500 hover:text-slate-700">Annuler</button>
              <button onClick={savePaieInfo} disabled={savingPaie} className="px-5 py-2.5 bg-indigo-600 hover:bg-indigo-500 text-white font-black text-xs uppercase rounded-xl transition flex items-center gap-2 disabled:opacity-50">
                {savingPaie ? <Loader2 className="w-4 h-4 animate-spin" /> : <CheckCircle className="w-4 h-4" />}
                Enregistrer
              </button>
            </div>
          </div>
              ...
              {loadingPayslips ? (
                ...
                payslips.map((p) => (
                  <tr key={p.id} className="hover:bg-slate-50/50 dark:hover:bg-slate-800/30 transition-colors">
                    ...
                    <td className="px-6 py-4 text-right">
                      <button onClick={() => printPayslip(p)} className="flex items-center gap-1.5 px-3 py-1.5 bg-indigo-50 dark:bg-indigo-500/10 hover:bg-indigo-100 text-indigo-600 dark:text-indigo-400 rounded-xl text-[10px] font-black uppercase tracking-widest transition ml-auto">
        ...
        <div ref={printRef}>
          {printSlip && (
            <BulletinPaiePDF
              ...
              employee={{
                nom: printSlip.personnel?.nom || printSlip.personnel_id,
                fonction: printSlip.fonction || printSlip.personnel?.role,
                matricule: printSlip.matricule || undefined,
                departement: printSlip.departement || undefined,
                cnss: printSlip.numero_cnss || undefined,
                dateEmbauche: printSlip.date_embauche
                  ? new Date(printSlip.date_embauche).toLocaleDateString('fr-FR')
                  : undefined,
