// ============================================================
// SCAN PRÉSENCE PERSONNEL — Pointage entrée du personnel (badge QR)
// Scanné par un surveillant/secrétaire à l'entrée, jamais par
// l'enseignant lui-même. N'utilise PAS le store Zustand ni /api/sync
// (cette donnée est hors du flux synchronisé élèves) — voir
// staffAttendanceApi.ts / staffAttendanceController.js.
// ============================================================
...
import { Camera, AlertTriangle, CheckCircle2, UserCheck, X } from 'lucide-react';
    ...
    <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-fade-in">
        <div className={`w-full max-w-sm rounded-[2.5rem] border-4 p-8 text-center shadow-2xl ${dejaPointe ? 'border-amber-400 bg-white' : 'border-emerald-400 bg-white'}`}>
            ...
            <div className={`py-3 px-6 rounded-2xl font-black text-lg ${dejaPointe ? 'bg-amber-100 text-amber-700' : 'bg-emerald-100 text-emerald-700'}`}>
                {dejaPointe ? 'DÉJÀ POINTÉ' : `ENTRÉE À ${heure}`}
            </div>
        </div>
    </div>
);

export const ScanPresenceEnseignant: React.FC = () => {
    ...
    const registerEntree = useCallback(async (personnelId: string) => {
        ...
        try {
            ...
            if (status.hasIn) {
                ...
                await staffAttendanceApi.scan(personnelId, 'in');
                playSuccessSound();
    ...
    const startCamera = async () => {
        ...
        try {
            const html5QrCode = new Html5Qrcode('reader-staff-in');
            html5QrCodeRef.current = html5QrCode;
            await html5QrCode.start(
                { facingMode: { exact: 'environment' } },
                { fps: 25, qrbox: { width: 250, height: 250 } },
                (decodedText) => { if (!isScanningPaused.current) registerEntree(decodedText); },
                () => {}
            );
        } catch (err) {
        ...
        <div className="max-w-3xl mx-auto space-y-6">
            <div className="bg-gradient-to-r from-teal-600 to-emerald-700 rounded-2xl p-6 text-white">
                <div className="flex items-center gap-3 mb-2">
                    ...
                    <div>
                        <h2 className="text-xl font-bold">Pointage entrée — Personnel</h2>
                        <p className="text-teal-100 text-sm">Scan du badge en début de journée de cours</p>
                    </div>
                </div>
            </div>
            ...
            <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
                <div className="p-4 border-b border-gray-100 flex items-center justify-between">
                    ...
                    <button
                        ...
                        className={`px-4 py-2 rounded-xl text-xs font-bold flex items-center gap-1.5 transition-all ${cameraActive ? 'bg-red-100 text-red-600 hover:bg-red-200' : 'bg-teal-600 text-white hover:bg-teal-700'}`}
                    >
                ...
                {cameraActive && (
                    <div className="relative bg-black w-full" style={{ minHeight: '300px' }}>
                        <div id="reader-staff-in" className="w-full h-full"></div>
                        {flashError && (
...
export default ScanPresenceEnseignant;
