...
// ── Carte verso — affichage écran ──
// Texte libre partagé avec les cartes élèves (Paramètres > Verso des cartes).
const CarteVersoPersonnel: React.FC<{ schoolName: string; schoolLogo: string | null; versoTexte: string }> = ({ schoolName, schoolLogo, versoTexte }) => (
    <div style={{
        ...
        <div style={{ flex: 1, padding: '16px 18px', display: 'flex', flexDirection: 'column' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 10 }}>
                <div style={{
                    ...
                    border: `1px solid ${TEAL}`, display: 'flex', alignItems: 'center',
                    justifyContent: 'center', padding: 2, flexShrink: 0,
                }}>
                    {schoolLogo
                        ? <img src={schoolLogo} style={{ maxWidth: '100%', maxHeight: '100%', objectFit: 'contain' }} />
                        : <span style={{ color: TEAL, fontWeight: 900, fontSize: 8 }}>ID</span>}
                </div>
                <span style={{ color: '#111827', fontWeight: 800, fontSize: 10, textTransform: 'uppercase' }}>{schoolName}</span>
            </div>
            <p style={{ flex: 1, color: '#374151', fontSize: 9.5, lineHeight: 1.5, whiteSpace: 'pre-wrap', overflow: 'hidden', margin: 0 }}>
                {versoTexte}
            </p>
...
const generateCartesPDF = async (
    ...
    // ── PAGES VERSO — mêmes positions que le recto (impression recto-verso, bord
    // long) : voir CarteScolaire.tsx pour le même mécanisme côté élèves. ──
    if (versoTexte.trim()) {
        const cardsPerPage = cols * rowsMax;
        const nbRectoPages = Math.ceil(personnel.length / cardsPerPage);
        const versoLines = doc.splitTextToSize(versoTexte.trim(), cardW - 6);

        for (let pageIdx = 0; pageIdx < nbRectoPages; pageIdx++) {
            doc.addPage();
            const cardsOnThisPage = Math.min(cardsPerPage, personnel.length - pageIdx * cardsPerPage);
            ...
            <div className="bg-white rounded-[24px] shadow-[0_2px_20px_rgba(0,0,0,0.04)] p-6">
                ...
                <div className="mt-5 p-4 bg-slate-50 rounded-[16px] flex items-start gap-3">
                    ...
                    <p className="text-sm font-medium text-slate-600 leading-relaxed">
                        ...
                        {carteVersoTexte.trim() && ' Un verso personnalisé est configuré (Paramètres > Verso des cartes) : les pages verso sont ajoutées après chaque page de rectos, prêtes pour une impression recto-verso.'}
                    </p>
                </div>
            </div>
                        ...
                        <div className="flex flex-col lg:flex-row gap-10 items-start">
                            ...
                            {carteVersoTexte.trim() && (
                                <div className="flex-shrink-0">
                                    ...
                                    <div className="p-4 bg-slate-50 rounded-[24px]">
                                        <div style={{ boxShadow: '0 25px 50px -12px rgba(0,0,0,0.25)', display: 'inline-block' }} className="transition-transform hover:scale-[1.02] duration-500">
                                            <CarteVersoPersonnel schoolName={schoolName} schoolLogo={schoolLogo} versoTexte={carteVersoTexte} />
                                        </div>
                                    </div>
                                </div>
                            )}
